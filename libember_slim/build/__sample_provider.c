...
static int handleClient(SOCKET sock)
{
   byte buffer[64];
   int len;
   const int rxBufferSize = 1024; // max size of unfamed package
   struct timeval timeout = {0, 100 * 1000}; // 50 milliseconds timeout for select()
   const int noDelay = 1;
   ...
   while(true) {
		len = recv(sock, (char *)buffer, sizeof(buffer), 0);
		//len = read(sock, (char *)buffer, sizeof(buffer));
		if (len == 0) {
			return -1;
		} else if (len > 0) {
        	printf_s("received %d bytes\n", len);
        	glowReader_readBytes(pReader, buffer, len);
       } else {
...
static void acceptClient(int port)
{
   ...
   if(clientSock > 0)
   {
	...
	while(1) {
		...
		pthread_t pth;
		//printf("newsock:%d\n", newsock);
		pthread_create(&pth, NULL, server, (void *)&newsock);
	}
#endif
