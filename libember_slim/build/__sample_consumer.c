...
static void sock_input(int sock, GlowReader *pReader) {
	char buf[BUFSIZ];
	int len;
    bool cont = true;
	while(cont) {
        ...
        if (len> 0) {
			pthread_mutex_lock(&mutex);
        	glowReader_readBytes(pReader, buf, len);
			pthread_mutex_unlock(&mutex);
        } else {
			cont = false;
		}
...
static void inputs(Session *pSession, GlowReader *pReader) {
	pthread_t pid;
	pthread_create(&pid, NULL, keyboard_input, (void *)pSession);

	sock_input(pSession->sock, pReader);	
}
#endif
...
static void run(Session *pSession)
{
   ...
   while(isQuit == false)
   {
      ...
      else
      {
         ...
         if(fdsReady < 0) // socket is ready to read
         {
			 printf("*** fdsReady:%d\n", fdsReady);
